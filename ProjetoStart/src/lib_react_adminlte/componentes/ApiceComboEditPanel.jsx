...
export default class ApiceComboEditPanel extends React.Component {
  ...
  mostrarPesquisa() {
    this.modal.show(() => {
      if (this.props.value) {
        ...
        this.modal.selecionarRegistroByCampo(this.props.chave,
           this.props.value);
      }
    });
  }
  ...
  renderColunasPesquisa() {
    return this.props.colunas ? this.props.colunas.map((x) => (
      <Column key={x.props.field} className={x.props.className} style={x.props.style} header={x.props.header} field={x.props.field} />
    )) : [];
  }
