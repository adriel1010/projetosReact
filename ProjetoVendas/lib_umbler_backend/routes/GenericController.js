...
module.exports = (class GenericController {
  ...
  /**
   * Recupera o valor e nome de todos os custom fields do parâmetro.
   * @param {string[]} nomes Array de nomes de custom fields
   */
  async getCustomFields(dados) {
    const retorno = [];
    for (let i = 0; i < dados.length; i++) {
      const ret = await dbUtils.query(' select * from custom_field ' +
        ' where nome = "' + dados[i].nome + '" ' +
        ' and aux = "' + dados[i].aux + '"');
      if (ret && ret.length) {
        retorno.push(ret[0]);
      } else {
  ...
  /**
   */
  async setCustomFields(dados) {
    for (let i = 0; i < dados.length; i++) {
      const ret = await dbUtils.query(' select * from custom_field ' +
        ' where nome = "' + dados[i].nome + '" ' +
        ' and aux = "' + dados[i].aux + '"');
      if (ret && ret.length) {
        // O Custom field já existe, apenas atualiza ele:
        await dbUtils.query(`update custom_field 
                                set valor = '${dados[i].valor}',
                                    versao = versao + 1,
                                    aux = '${dados[i].aux}'
                             where nome = '${dados[i].nome}' 
                                and aux = '${dados[i].aux}'`);
      } else {
        // Teremos que inserir o custom field
        await dbUtils.query(`insert into custom_field 
                                set nome = '${dados[i].nome}', 
                                    valor = '${dados[i].valor}',
                                    aux = '${dados[i].aux}',
                                    versao = 1`);
      }
    ...
    if (query === 'apice-combo-edit-query') {
      ...
      return await dbUtils.queryFindOne(`
        ...
        where ${chave} = ${filtro.codigo}
          ...
          ${filtro.ativo ? 'and dt_hr_exclusao is null' : ''}
      `);
    }
