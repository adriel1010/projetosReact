...
module.exports = (class GenericRoutes {
  ...
  /**
   * Ativa uma rota de upload específica para a implementação de Routes do projeto.
   */
  ativarRotaUpload(rota, callback) {
    this.server.post(rota, multer().any(), callback);
  }
  ...
  /**
   */
  ativarRotasCustomFields(server) {
    server.post('/get_custom_fields', apiceAuth.validate, async (req, res) => {
      try {
        const ret = await this.controller.getCustomFields(req.body.dados);
        res.send({
          status: true,
          dados: ret,
        });
    ...
    server.post('/set_custom_fields', apiceAuth.validate, async (req, res) => {
      try {
        await this.controller.setCustomFields(req.body.dados);
        res.send({
          status: true,
        });
      } catch (ex) {
  ...
  ativarRotas(server) {
    ...
    this.ativarRotasCustomFields(server);
