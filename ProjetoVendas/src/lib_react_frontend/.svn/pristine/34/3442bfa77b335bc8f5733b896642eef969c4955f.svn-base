...
export default class GenericApi {
  ...
  static async post(rota, params) {
    ...
    const res = await axios.post((this.getUrl ? 
      this.getUrl() : global.app.urlApi) + rota, { ...params });
    return res.data;
  }
  ...
  /**
   * Recupera os custom fields de acordo com o array de dados.
   */
  static async getCustomFields(dados) {
    return await this.post('/get_custom_fields', { dados });
  }

  /**
   */
  static async setCustomFields(dados) {
    return await this.post('/set_custom_fields', { dados });
  }
