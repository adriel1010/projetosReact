  ...
  render() {
    return (
      <div id={this.props.id} className={'box ' +
            (this.props.collapsible && !this.props.openByDefault ? 'collapsed-box ' : '') +
        ...
        {this.props.title &&
          <div className={' box-header hand with-border ' + this.props.headerClass}
            ...
            {this.props.subtitle ? 
            <span><br/> <h4 className="box-title">{this.props.subtitle}</h4></span>
            : null }
            {/* Icones: */}
        ...
        {/* Conteudo G.O.D: */}
        <div className={"box-body "+this.props.bodyClass}>
          {this.props.children}
        </div>
      </div>
    );
